...
for seed in SEEDS:
    ...
    E,logit,T,lab=run_arms(C,seed,cfg=json.load(open('p4_final_config.json'))['mobile']); out=evaluate(C,logit); pol,choice=policies(C,logit); out.update(pol)
...
json.dump(res,open(f'p4_lamda_v5_{CFG}_n{NTR}.json','w'))
def summ(res,arms):
    for a in arms:
